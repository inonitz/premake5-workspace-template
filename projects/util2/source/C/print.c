...
{
    if(s_currlogbuffer) {
        fclose(s_currlogbuffer);
    }
    if(s_currlogbuffer != s_logbuffer) {
        fclose(s_logbuffer);
    }
...
static void util2_init_state_once()
{
    ...
    s_logbuffer = fopen("printlog.txt", "w");
    s_currlogbuffer = UTIL2_PRINT_LOG_TO_FILE == 1 ? s_logbuffer : stdout;
...
void util2_fputs(FILE* write_to, const char* str) {
    fputs(str, write_to); /* thread safe as long as handle is valid */
    return;
}


void util2_puts(const char* str) {
    fputs(str, s_currlogbuffer); /* thread safe as long as handle is valid */
    return;
}


void util2_setfs(FILE* newfilestream) {
    s_currlogbuffer = (newfilestream == NULL) ? s_logbuffer : newfilestream;
    return;
}
